...
params.adapterfa = "TruSeq3-PE.fa"
...
if (params.adapter_trim_tag == "Y" ) {
    ...
    process Trim_Adapters {
        ...
        set pair_id, "*.clipped.up.fastq.gz"  into UPtrimmedfastq

        script:
        """
        $params.trimmomatic  PE -threads $params.cpu -phred33  ${pair_id}_R1${params.reads_suffix}  ${pair_id}_R2${params.reads_suffix}  ${pair_id}_1.clipped.fastq.gz  ${pair_id}_1.clipped.up.fastq.gz  ${pair_id}_2.clipped.fastq.gz  ${pair_id}_2.clipped.up.fastq.gz  ILLUMINACLIP:$params.adapterfa:2:30:10 > trimmomatic.log 2>&1
        """
    }
} 
...
process FASTQC {
    ...
    $params.fastqc  --noextract -f  fastq    -o   ./   ${pair_id}_2.clipped.fastq.gz     >    ${pair_id}_2.fastqc.log   2>&1
    """
...
process RSEMwithSTAR {
    ...
    """    
    $params.rsem  -p  $params.cpu   --sort-bam-by-coordinate  --keep-intermediate-files   --forward-prob 0  --paired-end  --star   --star-gzipped-read-file       ${pair_id}_1.clipped.fastq.gz   ${pair_id}_2.clipped.fastq.gz   index_folder/GRChg38_UCSC   ${pair_id}.RSEM_Output  >  ${pair_id}.RSEM.log  2> ${pair_id}.RSEM.err.log
...
if (params.TE_pipeline_run_tag == "Y" ) {
    process squireFetch  {
     ...
     process squireMap  {
         ...
         """
         squire  Map  -1   ${pair_id}_1.clipped.fastq.gz   -2  ${pair_id}_2.clipped.fastq.gz      -o  ${pair_id}.squire_map_folder  -r  150  -n  ${pair_id}  -b  hg38   --gtf  squire_fetch/hg38_refGene.gtf  -p  $params.cpu  -v  >squire_Map_${pair_id}.log  2>squire_Map_${pair_id}.err.log
         """
