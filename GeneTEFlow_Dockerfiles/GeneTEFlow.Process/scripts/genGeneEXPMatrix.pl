...
foreach my $sample (@sampleinfo)
{
	...
	my $generesult = "$rsemdatadir/$sample".".RSEM_Output.genes.results";
	
	my $degeneresultln = "$samplename.genes.results";
...
{
	my $outputfile = "all.sample.Counts.genes.results.txt";
#	`~/rsem-generate-data-matrix $allresultfile  > $outputfile  ` ; 

        $offsite = 4;       # for new file formats, expected_count      
  
        rsemGenerateDataMatrix($allresultfile,$outputfile)
}
elsif ( $datatype eq "tpm" )
{
	
	my $outputfile = "all.sample.TPM.genes.results.txt";
#        print $allresultfile."\n";
#	`~/rsem-generate-data-matrix.tpm $allresultfile  > $outputfile  ` ; 

        $offsite = 5; # for new file formats; TPM column
