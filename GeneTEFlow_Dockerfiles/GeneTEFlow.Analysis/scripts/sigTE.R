...
output.sigsum.file<-c("all.sample.sigTE.sum.txt")
...
{
	genelist.filename<-paste(names(siggeneslist)[k], ".sigTElist", sep="")
...
for ( i in 1:length(compareSubGroup))
{
	...
	outputunion.sigfile<-paste("all.sig.TE.subset.", subsetGroup, ".tpm.txt", sep="")
	write.table(cbind("TEid"=row.names(data.exp.subset), data.exp.subset), outputunion.sigfile, quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)

	## generate graphs 
	grpahfile<-paste("all.sig.TE.subset",subsetGroup, fdr.cutoff, ratio.cutoff, "sum.pdf", sep="." )
	pdf(file=grpahfile)
